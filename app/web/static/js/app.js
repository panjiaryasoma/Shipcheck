...
function markdownFinding(finding) {
  const lines = [
    `### ${markdownText(finding.requirement_id || "Requirement")} · ${markdownText(finding.status || "UNKNOWN")}`,
    "",
    markdownText(finding.requirement_text || "No requirement text reported."),
    "",
    `- **Type:** ${markdownText(finding.requirement_type || "not reported")}`,
    `- **Severity:** ${markdownText(finding.severity || "not reported")}`,
    "",
    "**Reason**",
    "",
    markdownText(finding.reason || "No reason reported."),
  ];

  if (Array.isArray(finding.evidence) && finding.evidence.length) {
    lines.push("", "**Evidence**", "");
    finding.evidence.forEach((item) => {
      lines.push(`- **Source:** ${markdownText(item.source || "evidence")}`);
      if (item.path_or_url) {
        lines.push(`  - Location: \`${markdownCode(item.path_or_url)}\``);
      }
      if (item.observed_value) {
        lines.push(`  - Observed: ${markdownText(item.observed_value)}`);
      }
    });
  } else {
  ...
  if (finding.recommended_action) {
    lines.push(
      ...
      "**Recommended action**",
      "",
      markdownText(finding.recommended_action),
    );
  }
...
function reportAsMarkdown(report) {
  ...
  const lines = [
    ...
    "",
    `> Final disposition: **${markdownText(report.final_disposition || "UNKNOWN")}**`,
    "",
    "## Inspection",
    "",
  ...
  if (findings.length) {
    ...
    lines.push("No requirement findings were reported.");
  }
...
function downloadReport() {
  ...
  const markdown = reportAsMarkdown(latestReport);
  const blob = new Blob([markdown], {type: "text/markdown;charset=utf-8"});
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `${latestReport.inspection_id || "shipcheck-report"}.md`;
